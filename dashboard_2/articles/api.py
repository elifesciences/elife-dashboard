...
class CurrentArticlesAPIView(APIView):
	...
	def get(self, request, format='json'):
		...
		for article_id in Property.find.latest_articles():
			# TODO remove from here
			# events = Event.objects.filter(article__article_identifier=article_id)

			latest_version = Article.versions.latest(article_id)
			# latest_version = 1
			current_articles[article_id] = Article.details.get(article_id, latest_version)
...
class ArticleDetailAPIView(APIView):
	...
	def get(self, request, article_id, format=None):
		...
		try:
			article = Article.objects.get(article_identifier=article_id)  # TODO prefetch rel data
			# article_data = {'article-id': article.id, 'article-identifier': article.identifier}
		except ObjectDoesNotExist as err:  # TODO complete
			return Response({}, status=status.HTTP_404_NOT_FOUND)

		data = {
			'id': article.article_identifier,
